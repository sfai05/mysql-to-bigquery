  ...
  _createClass(MySQLtoBigQuery, [{
    ...
    value: function _insertRecords(tableName) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this4.connection.query('SELECT * FROM ' + tableName + ';').stream().pipe((0, _eventStream.map)(function (data, cb) {
          return cb(null, _this4._toTSV(data));
        })).pipe(table.createWriteStream(options)).on('error', reject).on('complete', resolve);
      });
    }
  }, {
    key: '_toTSV',
    value: function _toTSV(data) {
      var _this5 = this;

      return (0, _lodash.trimLeft)(data.reduce(function (result, val) {
        return result + '\t' + _this5._escape(val);
      }, '') + '\n');
    }
  }, {
    key: '_escape',
